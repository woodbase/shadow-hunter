...
func _ready() -> void:
	...
	start_button.mouse_entered.connect(func(): AudioManager.play_ui("button_select"))
	quit_button.mouse_entered.connect(func(): AudioManager.play_ui("button_select"))
	start_button.focus_entered.connect(func(): AudioManager.play_ui("button_select"))
	quit_button.focus_entered.connect(func(): AudioManager.play_ui("button_select"))

	call_deferred("_setup_coop_selector")
...
func _setup_coop_selector() -> void:
	...
	for i in range(1, CoopManager.MAX_PLAYERS + 1):
		var btn := Button.new()
		btn.text = str(i)
